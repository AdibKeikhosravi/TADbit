...
def run(opts):
    ...
    elif opts.binless_bams:
        mreads = opts.binless_bams[0]
    else:
...
def populate_args(parser):
    ...
    normpt.add_argument('--binless_jobids', dest='binless_jobids', metavar="INT",
                        ...
                        Optional for binless normalization''')

    normpt.add_argument('--binless_bams', dest='binless_bams', metavar="STR",
                        nargs='+', default=None, type=str,
                        help='''Use as input several bam files.
                        \ne.g.: '--binless_bams file1.bam file2.bam.\n
                        Optional for binless normalization''')
    ...
    try:
        ...
        for r in bamfile.fetch(region=region,
            ...
            if crm1 == region and crm2 == region and start <= pos1 < end and start <= pos2 < end:
                ...
                e2 = bisect(rsites,pos1-1)
                e1 = max(0, e2 - 1)
                e1 = rsites[e1] + 1
                e2 = rsites[e2] + 1
                e4 = bisect(rsites,pos2-1)
                e3 = max(0, e4 - 1)
                e3 = rsites[e3] + 1
                e4 = rsites[e4] + 1
